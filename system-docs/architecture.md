...
> 最近更新：2026-06-07　|　对应版本：**v0.19**（文件夹门户重构完成：前后端打通，dev 分支）
...
| 后端代码 | 🟢 文件夹门户模型 + 双库 + 定时 + 保留 + 审计 + 分页 + 单服务发布（v0.17，70 测试） |
| 前端代码 | 🟢 管理端(文件夹树/数据集/数据文件/用户与部门/审计) + 用户端文件浏览器（v0.18–v0.19） |
...
## 4. 数据模型（v0.17 重构，迁移为准）

```
DataSource（Oracle/MySQL 连接，密码加密）
Folder（多层树，parent 自引用）──< DataFile（文件夹里的 Excel）
Dataset（SQL任务：datasource + sql + 目标文件夹 + 命名(前缀/日期格式) + 定时 + 保留）
        每次运行 → 在 target_folder 新增一个 DataFile
FolderShare：Folder → 部门 或 个人（递归覆盖子文件夹/文件）
Department ──< DepartmentMembership >── User（一人可多部门；无角色细分）
User 标志：is_superuser / is_assistant_admin / is_boss
AuditLog（登录/运行/下载留痕）
```

- **可见性唯一出口**：`apps/permission/services.py`
  - `can_view_folder/can_view_file`：超管/辅助管理员/老板→全部；否则"我的部门或本人"被授权了该文件夹或其任一祖先→可见；默认拒绝。
  - `visible_folders(user)`：被授权文件夹 + 其所有子孙。
- 详细字段见各 app 迁移文件。
