...
> 最近更新：2026-06-05　|　对应版本：**v0.09**（数据集 + SQL→Excel 核心闭环，dev 分支）
...
| 后端代码 | 🟡 安全核心 + 认证 + 数据模型/权限 + 数据源 + **数据集/SQL→Excel/下载**（v0.09，50 测试） |
| 前端代码 | 🟡 双布局 + 数据源页 + **数据集页(SQL/预览/运行)/执行记录(下载)**（v0.09，build/lint 通过） |
...
└── src/views/
    ├── admin/                  🟡 home / 数据源✅ / 数据集✅(SQL/预览/运行) / 执行✅(下载) / 目录·权限(占位)
