...
> 最近更新：2026-06-05　|　对应版本：**v0.08**（数据源管理 + 测试连接，dev 分支）
...
| 后端代码 | 🟡 安全核心 + 认证 + 数据模型/权限判定 + **数据源 API**（v0.08，42 测试通过） |
| 前端代码 | 🟡 双布局 + 角色分流 + **数据源管理页**（v0.08，build/lint 通过） |
...
└── src/views/
    ├── admin/                  🟡 home / 数据源(✅CRUD+测试) / 数据集·目录·权限·执行(占位)
